...
import Reliances from "../reliance.json";

class Reliance extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      stockChartXValues: [],
  ...
  fetchStock() {
    ...
    //     }
    {
      Reliances.map((reliance) => {
        stockChartXValuesFunction.push(reliance.Date);
        stockChartYValuesFunction.push(reliance.Open);
      });
    }
  ...
  render() {
    return (
      <div>
        <h1 style={{ color: "white" }}>Reliance</h1>
        <Plot
          data={[
            {
