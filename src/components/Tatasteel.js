...
import Tatasteels from "../tatasteel.json";

class Tatasteel extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      stockChartXValues: [],
  ...
  fetchStock() {
    ...
    //     }
    {
      Tatasteels.map((tatasteel) => {
        stockChartXValuesFunction.push(tatasteel.Date);
        stockChartYValuesFunction.push(tatasteel.Open);
      });
    }
  ...
  render() {
    return (
      <div>
        <h1 style={{ color: "white" }}>Tatasteel</h1>
        <Plot
          data={[
            {
