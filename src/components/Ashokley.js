...
import Ashokleys from "../ashokley.json";

class Ashokley extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      stockChartXValues: [],
  ...
  fetchStock() {
    ...
    //     }
    {
      Ashokleys.map((ashokley) => {
        stockChartXValuesFunction.push(ashokley.Date);
        stockChartYValuesFunction.push(ashokley.Open);
      });
    }
  ...
  render() {
    return (
      <div>
        <h1 style={{ color: "white" }}>AshokLey</h1>
        <Plot
          data={[
            {
