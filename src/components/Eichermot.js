...
import Eichermots from "../eichermot.json";

class Eichermot extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      stockChartXValues: [],
  ...
  fetchStock() {
    ...
    //     }
    {
      Eichermots.map((eichermot) => {
        stockChartXValuesFunction.push(eichermot.Date);
        stockChartYValuesFunction.push(eichermot.Open);
      });
    }
  ...
  render() {
    return (
      <div>
        <h1 style={{ color: "white" }}>Eichermot</h1>
        <Plot
          data={[
            {
